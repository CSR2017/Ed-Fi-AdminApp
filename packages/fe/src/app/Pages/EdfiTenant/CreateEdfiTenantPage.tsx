...
export const CreateEdfiTenantPage = () => {
  const popBanner = usePopBanner();
  const navToParentOptions = useNavToParent();
  const { sbEnvironmentId, teamId } = useTeamSbEnvironmentNavContext();
  const navigate = useNavigate();
  const postEdfiTenant = edfiTenantQueries.post({ sbEnvironmentId, teamId });
  const {
    register,
    setError,
    handleSubmit,
  ...
  return (
    <PageTemplate constrainWidth title={'Create tenant'} actions={undefined}>
      <Box w="form-width">
        <form
            ...
            postEdfiTenant
              .mutateAsync(
                { entity: data },
                {
                  onSuccess: (result) => {
                    navigate(
                      `/as/${teamId}/sb-environments/${result.sbEnvironmentId}/edfi-tenants/${result.id}`
                    );
                  },
